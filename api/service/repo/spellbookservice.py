...
class SpellbookService:
    ...
    def createSpellbookForCharacter(cls, characterDto: CharacterDTO):
        ...
        if spellTable is None:
            Logger.error(f"Class [{ClassService.get(characterDto.classId).name}] does not have a spell table. Please create it manually.")
            raise Exception("Class does not have a spell table. Please create it manually.")
